...
def run_workflow(prior_dist, data, sigma):
    with pm.Model() as model:
        # Choose alpha prior distribution based on prior_dist type
...
sigma1 = 0.1
posterior1 = run_workflow(prior0, data1, sigma1)


# Step 2: use posterior1 as prior for new data2
prior1 = EmpiricalDistribution(posterior1.samples)
data2 = np.random.normal(loc=2.5, scale=0.6, size=120)
sigma2 = 0.1
posterior2 = run_workflow(prior1, data2, sigma2)


# Step 3: chain again with posterior2 
prior2 = EmpiricalDistribution(posterior2.samples)
data3 = np.random.normal(loc=3.0, scale=0.7, size=120)
sigma3 = 0.1
posterior3 = run_workflow(prior2, data3, sigma3)
